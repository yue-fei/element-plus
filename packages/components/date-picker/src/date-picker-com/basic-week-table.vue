...
import { handleWeekLastDay, handleWeekOneDay } from '../utils'
  ...
  setup(props, ctx) {
    ...
    const handleClick = (event) => {
      ...
      if (!props.rangeState.selecting) {
        ctx.emit(
          'pick',
          { minDate: newDate, maxDate: handleWeekLastDay(tmpNewDate) },
          false
        )
        ...
        if (tmpNewDate >= props.minDate) {
          ctx.emit('pick', {
            minDate: handleWeekOneDay(props.minDate),
            maxDate: handleWeekLastDay(tmpNewDate),
          })
        } else {
          ctx.emit('pick', {
            minDate: handleWeekOneDay(tmpNewDate),
            maxDate: handleWeekOneDay(props.minDate),
          })
        }
